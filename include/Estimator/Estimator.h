...
public:
	...
	struct FeatureLine{
		...
		double ComputeError(const Eigen::Matrix4d& pose){            //https://blog.csdn.net/p942005405/article/details/125039343
			...
			double l12 = std::sqrt((lineP1(0) - lineP2(0))*(lineP1(0) - lineP2(0)) + (lineP1(1) - lineP2(1))*
																						(lineP1(1) - lineP2(1)) + (lineP1(2) - lineP2(2))*(lineP1(2) - lineP2(2)));        //计算线段上两个点的长度
			double a012 = std::sqrt(
							...
							* ((P_to_Map(0) - lineP1(0)) * (P_to_Map(1) - lineP2(1)) - (P_to_Map(0) - lineP2(0)) * (P_to_Map(1) - lineP1(1)))
							...
							+ ((P_to_Map(1) - lineP1(1)) * (P_to_Map(2) - lineP2(2)) - (P_to_Map(1) - lineP2(1)) * (P_to_Map(2) - lineP1(2)))
								* ((P_to_Map(1) - lineP1(1)) * (P_to_Map(2) - lineP2(2)) - (P_to_Map(1) - lineP2(1)) * (P_to_Map(2) - lineP1(2))));  //计算平行四边形的面积
			error = a012 / l12; //计算高（点到线的距离）
		}
	};

	/** \brief point to plan feature */
	//计算点到平面的距离  https://xiaotaoguo.com/p/paper-note-loam-math/
	struct FeaturePlan{
		Eigen::Vector3d pointOri;
		double pa;  //平面方程的系数
		double pb;//平面方程的系数
		...
		double ComputeError(const Eigen::Matrix4d& pose){
			Eigen::Vector3d P_to_Map = pose.topLeftCorner(3,3) * pointOri + pose.topRightCorner(3,1);   //把点转换到世界坐标系下
			error = pa * P_to_Map(0) + pb * P_to_Map(1) + pc * P_to_Map(2) + pd; //计算点到平面的距离
		}
	};
	...
	struct FeaturePlanVec{
		Eigen::Vector3d pointOri;  // 原始点
		Eigen::Vector3d pointProj;  // 投影点
		Eigen::Matrix3d sqrt_info;  // 信息矩阵
		double error;  // 误差
		bool valid;  // 是否有效
		...
		double ComputeError(const Eigen::Matrix4d& pose){  // 计算误差
			Eigen::Vector3d P_to_Map = pose.topLeftCorner(3,3) * pointOri + pose.topRightCorner(3,1);  // 将点转换到地图坐标系
			error = (P_to_Map - pointProj).norm();  // 计算误差
		}
	};
