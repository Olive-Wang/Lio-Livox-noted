...
//用于从一个点云中获取指定搜索点附近的邻近点，并进行主元分析（PCA）以获取主方向和相关信息
...
//对输入的点云数据进行筛选，从中提取符合特定条件的点，并将结果存储在 `outPoints` 数组中
...
//计算旋转矩阵（Rotation Matrix）以将两个归一化向量旋转到一致的方向
